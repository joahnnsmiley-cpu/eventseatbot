...
export default function ProfileLayout({ children, className }: ProfileLayoutProps) {
  return (
    <motion.div
      ...
      className={className ? `w-full max-w-[720px] mx-auto px-6 py-4 ${className}` : 'w-full max-w-[720px] mx-auto px-6 py-4'}
      style={{
        minHeight: '100%',
        background: 'linear-gradient(180deg, #0F0F0F 0%, #141414 50%, #0D0D0D 100%)',
        ...(className?.includes('profile-organizer-premium') ? {} : { fontFamily: "'Plus Jakarta Sans', -apple-system, BlinkMacSystemFont, sans-serif" }),
      }}
    >
      {children}
    </motion.div>
