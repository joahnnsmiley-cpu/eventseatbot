...
const EventCard: React.FC<EventCardProps> = ({ event, mode, onClick, selected = false }) => {
  ...
  const cardClassName = `relative w-full text-left rounded-lg overflow-hidden border bg-gray-100 transition-all ${selected ? 'border-blue-500 ring-2 ring-blue-500 ring-offset-2' : 'border-gray-200'} ${isClickable ? 'focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-offset-2 active:scale-[0.98] cursor-pointer hover:shadow-md' : 'cursor-default opacity-90'}`;
...
export const EventCardSkeleton: React.FC = () => (
  <div
    className="w-full rounded-lg overflow-hidden border border-gray-200 bg-gray-100 min-h-[120px] animate-pulse flex flex-col justify-end p-4"
    aria-hidden
  >
